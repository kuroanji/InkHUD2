#!/usr/bin/env python3
"""Generate font with Latin + Simplified Chinese characters."""
...
FONT_CJK = SCRIPT_DIR / "fonts/NotoSansSC-Regular.ttf"

# Output file (in same directory as script)
OUTPUT_PATH = SCRIPT_DIR / "ChineseFont18px.h"
FONT_NAME = "ChineseFont18px"

# Common Chinese characters file (HSK levels 1-6 + common chars)
HANZI_PATH = SCRIPT_DIR / "common_hanzi.txt"
...
def get_common_hanzi():
    """Load common Chinese characters from file."""
    hanzi_file = Path(__file__).parent / HANZI_PATH
    if not hanzi_file.exists():
        print(f"WARNING: {hanzi_file} not found, using default set")
        # Default: HSK 1-4 most common ~1500 chars
        return []
    ...
    with open(hanzi_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if ch and len(ch) == 1:
                chars.append(ch)
    print(f"  Loaded {len(chars)} Chinese characters")
    return chars
...
def get_all_codepoints():
    """Get all codepoints: ASCII + Chinese."""
    codepoints = set()
    ...
    # Chinese characters
    hanzi = get_common_hanzi()
    for ch in hanzi:
        codepoints.add(ord(ch))

    # If no file, add basic CJK Unified range (most common ~3000)
    if not hanzi:
        # Common Chinese chars U+4E00-U+9FFF (add subset)
        common_ranges = [
            (0x4E00, 0x4E50),  # Basic
            (0x5000, 0x5200),
            (0x6000, 0x6200),
...
def generate_font():
    ...
    with open(output_path, 'w') as f:
        f.write(f"// Auto-generated Chinese font - {len(glyphs)} glyphs\n")
